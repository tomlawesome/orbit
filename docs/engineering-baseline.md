...
| Document ingestion and scanning | **Mixed, now determined (#726)** | Live: `scripts/test-malware-scanner.sh` against real ClamAV; `scripts/test-tika-processor.mjs` against real Tika, both in isolation. The integrated smoke journey — the only real upload the `smoke` job drives is the mailbox one, `tests/e2e/v19-mail-collection.spec.ts:150` — **reaches live ClamAV**: `scanAndHoldImapAttachment` (`src/server/mail-in/imap-attachment-holding.ts:99-102`) calls the real socket client whenever `DOCUMENT_SCAN_MODE=required` (the default the smoke stack never overrides, `.env-orbit.example:84`), and fails closed with `scanner_disabled` rather than silently skip scanning if it is ever anything else; pinned at the unit level by the `required mode` cases in `src/server/mail-in/imap-attachment-holding.test.ts`. It **never reaches Tika**: `orbit-tika` only starts under the `processing` Compose profile (`docker-compose.yml:88`), which `scripts/ci/create-test-configuration.sh` never enables and `.env-orbit.example:62`/`:92` leave empty; and even with `TIKA_URL` set, the mailbox path never calls `extractTextWithTika` at all — only the on-demand `/api/documents/[id]/draft` and item-inspection endpoints do (`src/server/document-drafts.ts:104`, `src/server/item-document-inspection.ts:184`), and no smoke spec exercises either against a live backend (`v19-mail-review.spec.ts` mocks the whole review API). Fake: `src/server/documents/tika.test.ts:4` stubs `fetch`. The integrated Tika path stays unproven; `scripts/test-tika-processor.mjs` remains the only harness that owns Tika evidence, in isolation | #726 closes the "undetermined" question; a follow-up issue is needed only if the product decides the mailbox journey should call Tika |
