...
| Maintenance mode | **Live, with a caveat** | `tests/e2e/maintenance.spec.ts` opens a window through the real admin API, asserts the 503 screen for a stranger and a member and the pass for the administrator, and ends the window through the real API. `scripts/test-backup-restore.sh:423-449` runs `end-maintenance.sh` for real, but *enters* maintenance by hand-written SQL (`:409-411`), so the CLI drill never exercises the app's own `openMaintenanceWindow` | [#526](https://github.com/tomlawesome/orbit/issues/526) |
| Document ingestion and scanning | **Mixed, now determined (#726, #838)** | Live: `scripts/test-malware-scanner.sh` against real ClamAV; `scripts/test-tika-processor.mjs` against real Tika, both in isolation. The mailbox journey — `tests/e2e/v19-mail-collection.spec.ts:150` — **reaches live ClamAV**: `scanAndHoldImapAttachment` (`src/server/mail-in/imap-attachment-holding.ts:99-102`) calls the real socket client whenever `DOCUMENT_SCAN_MODE=required` (the default the smoke stack never overrides, `.env-orbit.example:84`), and fails closed with `scanner_disabled` rather than silently skip scanning if it is ever anything else; pinned at the unit level by the `required mode` cases in `src/server/mail-in/imap-attachment-holding.test.ts`. It never reaches Tika — the mailbox path never calls `extractTextWithTika` at all. Since #838 the smoke stack **also reaches Tika**, through a different journey: `scripts/ci/create-test-configuration.sh` sets `COMPOSE_PROFILES=processing` and `TIKA_URL=http://orbit-tika:9998`, and `tests/e2e/v19-document-extraction.spec.ts:47` uploads a real Chromium-rendered PDF (real, font-encoded page text, not a stub-friendly string) through `/api/households/[id]/item-document-inspection` (`src/server/item-document-inspection.ts:184`) and polls for `extracted: true`, the strongest signal that route's contract exposes since it never returns extracted text to the client. Pinned at the unit level by `src/server/item-document-inspection.test.ts:487`, which leaves the Tika adapter unmocked and fakes only `fetch`. Fake: `src/server/documents/tika.test.ts:4` stubs `fetch` for the adapter's own unit coverage. `/api/documents/[id]/draft` (`src/server/document-drafts.ts:104`) still calls Tika on-demand with no live evidence at all (`v19-mail-review.spec.ts` mocks the whole review API) | #726 and #838 close the "undetermined" question for the mailbox and item-upload journeys; a follow-up issue is needed only if the product wants live evidence for the document-draft review journey too |
| Mail and IMAP ingestion | **Fake evidence only** | The strongest evidence is `tests/e2e/v19-mail-collection.spec.ts:119`, a real SMTP delivery into the disposable GreenMail sidecar (`compose/docker-compose.acceptance.yml:58-68`) consumed by the real IMAP poller — substantial, but a sidecar. `tests/e2e/imap-review.spec.ts:73-89` and `v19-mail-review.spec.ts:77-93` mock the API outright. The integration suites never dial IMAP (`IMAP_HOST=imap.example.invalid`). No real provider anywhere | [#22](https://github.com/tomlawesome/orbit/issues/22) |
...
| v19 front end (`web/`) | **Proven** | `pnpm --filter orbit-web fidelity` runs in CI on any pull request touching `web/` (`publish-container.yml:333-392`), and `pnpm --filter orbit-web build` plus `scripts/check-v19-types.mjs` run on every pull request (`scripts/test-backend.sh:16-18`). `check-v19-types.mjs` is now a plain zero-error `svelte-check` gate with no per-file tolerance; `web/svelte-check-ceiling.json` is kept only as the empty historical record | [#624](https://github.com/tomlawesome/orbit/issues/624) (closed) |
