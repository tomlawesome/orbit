...
| CI, preview and promotion | V1-REL-01 | validation and promotion workflows | build-once exact-image preview flow, Gitflow source checks, dependency/secret policy, exact-image vulnerability/SPDX evidence, digest-bound attestations and immutable digest recording | **Partially proven:** protected CI evidence for the new supply-chain gate, representative versioned-release acceptance and completed stable promotion remain |
