...
5. `load_installer_ui` only sources `installer-ui.sh` from the resolved staging directory (the helper extracted from the resolved image's own digest) after confirming it is a regular, non-symlink file — never an existing on-disk deployment copy — per an explicit design comment that this ordering prevents sourcing an untrusted local copy before validation. — install.sh:175-190 — category: provenance/immutability — criticality: HIGH
...
5. Compose project name must match `^[a-z0-9][a-z0-9_-]*$` — configuration.sh:91-93 (`is_valid_compose_project_name`) and install.sh:502,510,531,542 (`derive_compose_project_name`).
