...
41. `configuration-migration-interrupted` fires only when the live `.env-orbit` fails `configure.sh --check` AND `.env-orbit.orbit-config.rollback` exists, is a regular non-symlink file at mode 600, and itself passes the identical `configure.sh --check` semantics — run against a disposable staged copy under `$TMPDIR` (never the installation directory, matching the existing `configure_check_stderr` precedent), since `configure.sh` has no `--file` flag. It replaces, never adds to, the plain `configuration-invalid`/`configuration-incomplete` finding for that run, so at most one configuration finding is ever reported; any other outcome (missing, symlink, wrong mode, or a failing isolated check) leaves diagnosis exactly as it was before this class existed. — repair.sh:1416-1474,1845-1862 — category: refusal/fail-closed — criticality: HIGH
