...
| POL-12 | Drawers retract on scroll | Scrolling down animates every open drawer (left/right/top) back to its closed edge; scrolling up leaves them alone. The document card is **not** retracted by scroll — being `position:fixed` it cannot scroll away, so it is dismissed by CON-20 instead | in v17; card exclusion recorded 2026-08-14 |
...
| CON-14 | 404, three ways | Three concepts were built side by side in v17: (1) gravity well — the black hole is the 0, lensed arch, Doppler disc, spaghettified inner 4, infalling page debris; (2) conjunction — the 0 is a ring planet, the 4s orbit off-axis and tumble, aligning legibly each cycle, line: "Not in this world…"; (3) uncharted — past the survey limit the digits are a constellation labelled in the chart's own grammar. **Decision (owner, 2026-08-12): the gravity well ships as the one served 404.** Conjunction and uncharted stay in the repo, unlinked from the served route — "you never know" | **decided** — gravity well serves; others retained |
...
| CON-17 | Sunset logout | The login is first light; the logout is the day ending well — the same limb world performing the sunset once on load: the sun sinks, the scattering cools amber→ember→indigo, the stars come out and take over, an ember rim holds. Dry copy only ("SIGNED OUT", a Sign back in pill). Bookends for every session | in v18 |
| CON-19 | The mark, ratified | **Owner decision 2026-08-13 (final):** Orbit's mark is exactly three FLAT geometric shapes — the year ring, the planet riding it, and a plain filled circle at the centre (the household). No gradients, no glow, no 3D, at any size, in any medium; colour comes from the theme tokens; the planet may drift slowly on live surfaces (the only permitted motion). The word always sets plain. The three-arc mark retires; all bespoke wordmark treatments are dropped. Sheet: mockups-brand-decision.html. **Amended 2026-08-14 (owner):** the sign-in hero is an exception — the ring is drawn large with the word `orbit` set plain *inside* it, centred on the ring's own centre, and without the filled centre shape (the word occupies that space). The rule is unchanged everywhere else: three flat shapes, nothing else inside the ring, the word plain beside or under the glyph. | **ratified**, amended for the sign-in hero |
| CON-18 | Clean flights | v18 refinement of CON-13 per owner: the departing chart leaves COMPLETELY with the camera (no low-opacity remnant trailing like a catch-up); the destination's dial then performs the full POL-1 fanfare arrival at centre. The create drawer gains a scrim (sky dims beneath while open) and its star-handle becomes an explicit chip reading close; Escape/click-outside/scroll all retract it | in v18; light dismiss widened to every transient surface 2026-08-14, see CON-20 |
| CON-20 | Light dismiss | One rule for every transient surface on home — status drawer, key drawer, create drawer, document card, account panel. Click outside closes; Escape closes; opening one closes the others, so only ever one sits over the hero. The scrim stays create-only (CON-18): nothing else dims the sky. The document card also takes `overscroll-behavior:contain`, or reaching the end of a long list chains the scroll to the page and dismisses the card mid-read | owner 2026-08-14, issue #416 |
