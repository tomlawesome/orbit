...
// This file spawns real awk and bash (a generated install.sh driver); a
...
function execDocker(args: string[]): string | null {
  try {
    const result = execFileSync("docker", args, { encoding: "utf8", env: stubEnv, ...processGuard() });
...
function runDriver(...args: string[]): { status: number; stdout: string; stderr: string } {
  const result = failOnProcessDeadline(spawnSync("bash", [driverPath, ...args], { encoding: "utf8", env: stubEnv, ...processGuard() }), { label: "runDriver" });
