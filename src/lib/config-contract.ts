...
// The one definition of a 256-bit hexadecimal secret, as generated by
// `openssl rand -hex 32`. Three places used to restate it differently and
// disagree (issue #578): configure.sh demanded 64 hex characters, this
// module's field schema demanded 64 hex or empty, and the runtime loader
// accepted any string of 32 or more characters — so an instance could start
// on a value its own configure step refused. Every layer now resolves here:
// the field schema and the TypeScript configure engine import it, the
// runtime loader imports it, and scripts/configure.sh restates the same
// pattern under a test (src/lib/session-secret.contract.test.ts) that fails
// if any of them drifts.
export const SECRET_HEX256_PATTERN = /^[0-9a-fA-F]{64}$/;

/** The single rule for whether a value may be used as the session secret. */
 ...
 * scripts/configure.sh's secret_file_format_message produces the identical
 * sentence; session-secret.contract.test.ts fails if they diverge.
 */
export function secretFileFormatMessage(path: string): string {
  const name = path.slice(path.lastIndexOf("/") + 1);
  const rotation = name === "session-secret" ? ` ${SESSION_SECRET_ROTATION_WARNING}` : "";
  return `${path} does not contain a valid 256-bit hexadecimal secret: it ${SECRET_HEX256_REQUIREMENT}.${rotation}`;
}

/** The refusal an operator sees when the running instance is handed a bad session secret. */
export const SESSION_SECRET_RUNTIME_MESSAGE =
  `SESSION_SECRET ${SECRET_HEX256_REQUIREMENT}. ${SESSION_SECRET_ROTATION_WARNING}`;
...
export const envOrbitSchema = z
  .object({
    ...
    ORBIT_CONFIG_APPLIED_DIGEST: z
      ...
      .optional(),
    // Blank is "not set in this file" (the deployment may be file-backed
    // through SESSION_SECRET_FILE); any non-blank value must satisfy the one
    // rule above, which the runtime loader enforces identically.
    SESSION_SECRET: z
      .string()
      .refine((v) => v === "" || isValidSessionSecret(v), { message: SESSION_SECRET_RUNTIME_MESSAGE })
      .optional(),
    DOCUMENT_KEK: z
      .string()
