...
export function HouseholdRecoveryPrompt({ households, csrfToken, onCreate }: { households: RecoverableHousehold[]; csrfToken: string; onCreate(): void }) {
  const [busyId, setBusyId] = useState<string | null>(null);
  const [message, setMessage] = useState("");
  async function restore(household: RecoverableHousehold) {
    setBusyId(household.id); setMessage("");
  ...
  return <><button className="editor-scrim" type="button" tabIndex={-1} aria-hidden="true" /><section className="household-onboarding household-recovery-prompt" role="dialog" aria-modal="true" aria-labelledby="recovery-title"><div className="onboarding-art" aria-hidden="true"><span>Your households</span><p>Start fresh,<br />or bring one back.</p></div><div className="onboarding-form"><header><div><p>No active households</p><h2 id="recovery-title">Where would you like to begin?</h2></div></header><p className="onboarding-intro">Create a new household, or restore one you previously removed. Removed households stay hidden from everyone until restored.</p><button className="wizard-primary" type="button" onClick={onCreate}>Create household</button>{households.length > 0 && <div className="recovery-options"><strong>Restore a removed household</strong>{households.map((household) => <article key={household.id}><div><b>{household.name}</b><small>Available until {new Intl.DateTimeFormat("en-GB", { dateStyle: "medium" }).format(new Date(household.deleteAfter))}</small></div><button type="button" onClick={() => void restore(household)} disabled={busyId === household.id}>{busyId === household.id ? "Restoring…" : "Restore"}</button></article>)}</div>}{message && <p className="member-message" role="alert">{message}</p>}</div></section></>;
}
