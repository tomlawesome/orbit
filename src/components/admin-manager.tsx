...
interface Operations {
  ...
  documentJobs: Array<{ id: string; kind: string; status: DocumentJobStatus; attempts: number; lastErrorCode: string | null; createdAt: string; updatedAt: string }>;
  ...
  return <div className="settings-content">
    <section>
      ...
      {operations ? <>
        ...
        <div className="admin-operation-actions"><button type="button" onClick={() => void testSmtp()} disabled={smtpTestState === "sending" || operations.providers.smtp !== "configured"}>{smtpTestState === "sending" ? "Testing SMTP…" : "Test SMTP"}</button>{smtpTestState === "success" && <span role="status">SMTP test passed.</span>}</div>
...
function JobList({ title, empty, jobs, busy, onAction }: { title: string; empty: string; jobs: Array<{ id: string; status: DeliveryStatus | DocumentJobStatus; attempts: number; lastErrorCode: string | null; updatedAt: string; channel?: string; kind?: string }>; busy: string | null; onAction: (id: string, action: JobAction, status: DeliveryStatus | DocumentJobStatus) => void }) {
  return <div className="admin-job-group"><h4>{title}</h4>{jobs.length ? <ul className="admin-job-list">{jobs.map((job) => { const isDelivery = Boolean(job.channel); const busyKey = `${isDelivery ? "deliveries" : "document-jobs"}:${job.id}`; const canRetry = isDelivery ? ["failed", "cancelled"].includes(job.status) : job.status === "failed"; const canDiscard = isDelivery ? ["pending", "retry", "failed"].includes(job.status) : job.status === "failed"; return <li key={job.id}><span><strong>{job.channel ?? job.kind} · {job.status}</strong><small>{job.attempts} attempt{job.attempts === 1 ? "" : "s"} · {formatDate(job.updatedAt)}{job.lastErrorCode ? ` · ${job.lastErrorCode}` : ""}</small></span>{(canRetry || canDiscard) && <span className="admin-job-actions">{canRetry && <button type="button" disabled={busy !== null} onClick={() => onAction(job.id, "retry", job.status)}>{busy === busyKey ? "Working…" : "Retry"}</button>}{canDiscard && <button type="button" disabled={busy !== null} onClick={() => onAction(job.id, "discard", job.status)}>Discard</button>}</span>}</li>; })}</ul> : <p className="member-message">{empty}</p>}</div>;
}
