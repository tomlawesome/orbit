...
describe("document-assisted item inspection boundary", () => {
  it("authorizes inspection and creates no durable item, document, or audit before explicit submit", async () => {
    ...
    const payload = await response.json() as { extracted: boolean; suggestions: Array<{ field: string; value: string; source: string; confidence: string }>; proposal?: unknown; text?: unknown };
    expect(payload).toMatchObject({ extracted: false });
