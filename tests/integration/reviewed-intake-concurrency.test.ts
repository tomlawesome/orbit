...
describe("reviewed intake PostgreSQL idempotency boundaries", () => {
  ...
  it("leaves purge state alone when the winner has already purged the attachment", async () => {
    // #722: the losing caller's purge fails because the winner already removed
    // the private copy and cleared the flag. Re-stamping purgePending and a
    // failure code advertises outstanding purge work that is already done.
    const fixture = await createIntegrationFixture("reviewed-loser-purge-state");
    const targetItemId = randomUUID();
    await getDb().insert(items).values({ id: targetItemId, householdId: fixture.household.id, sectionId: fixture.section.id, title: "Loser purge state target", currency: "GBP" });
    const bytes = syntheticPdf("loser purge state");
    const receiptId = randomUUID();
    const held = await scanAndHoldImapAttachment({ bytes, filename: "loser-purge-state.pdf", declaredMediaType: "application/pdf", recipientUserId: fixture.users.member.id, receiptId });
    const document = await uploadItemDocument({
      userId: fixture.users.member.id, householdId: fixture.household.id, itemId: targetItemId,
      filename: "loser-purge-state.pdf", body: new ReadableStream({ start(controller) { controller.enqueue(bytes); controller.close(); } }),
      declaredBytes: bytes.length, documentId: held.id,
    });
    const [receipt] = await getDb().insert(imapIngestionMessages).values({
      id: receiptId,
      mailbox: "private", mailboxUidValidity: "loserpurge", mailboxUid: 7, contentSha256: randomUUID().replaceAll("-", ""),
      recipientAliasSha256: "loserpurge", userId: fixture.users.member.id, householdId: fixture.household.id,
      status: "pending_review", expiresAt: new Date(Date.now() + 86_400_000), receiptStatus: "pending",
    }).returning({ id: imapIngestionMessages.id });
    await getDb().insert(imapIngestionAttachments).values({
      id: held.id, messageId: receipt.id, displayName: held.displayName, mediaType: held.mediaType,
    ...
    const input = {
      ...
      action: "attach_existing" as const,
      targetItemId,
      item: { title: "Loser purge state target", currency: "GBP", status: "active" },
      attachmentIds: [held.id],
    };
    // Same seam as the concurrent-completion case above: the winner purges the
    // private copy, clears the flag and completes the receipt while this caller
    // is inside its own purge attempt, which therefore fails.
    setImapHoldingPurgeImplementationForTests(async (storageKey) => {
      setImapHoldingPurgeImplementationForTests(undefined);
      await purgeHeldImapAttachment(storageKey);
      await getDb().update(imapIngestionAttachments).set({ purgePending: false, purgeFailureCode: null }).where(eq(imapIngestionAttachments.id, held.id));
    ...
    try {
      ...
      const [attachment] = await getDb().select({
        ...
        purgePending: imapIngestionAttachments.purgePending,
        purgeFailureCode: imapIngestionAttachments.purgeFailureCode,
        purgeAttempts: imapIngestionAttachments.purgeAttempts,
      }).from(imapIngestionAttachments).where(eq(imapIngestionAttachments.id, held.id));
      // The winner's purge stands: no pending flag, no failure code, and no
      // attempt counted for a purge this caller never had left to do.
      expect(attachment).toMatchObject({ status: "assigned", purgePending: false, purgeFailureCode: null, purgeAttempts: 0 });
    } finally {
      setImapHoldingPurgeImplementationForTests(undefined);
      await fixture.cleanup();
    }
